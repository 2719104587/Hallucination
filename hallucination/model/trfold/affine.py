...
class Frame:
    def __init__(self, rots, trans):
        ...
        if self.rots is None and self.trans is None:
            ...
            self.trans = Frame.identity_trans(
                self.rots.shape[:-2],
                self.rots.dtype,
                self.rots.device,
    ...
    def identity(shape, dtype, device, requires_grad=False):
        return Frame(
            Frame.identity_rot(shape, dtype, device, requires_grad),
            Frame.identity_trans(shape, dtype, device, requires_grad),
        )
    ...
    def from_flat12(t):
        assert t.shape[-1] == 12
        rots, trans = torch.split(t, [9, 3], dim=-1)
        rots = torch.reshape(rots, (*rots.shape[:-1], 3, 3))
        return Frame(rots, trans)

    def to_flat12(self):
        tensor = self.rots.new_zeros((*self.shape, 12))
        tensor[..., :9] = torch.reshape(self.rots, (*self.shape, 9))
        tensor[..., 9:] = self.trans
        return tensor

    @staticmethod
    def from_tensor(t):
        return Frame.from_4x4(t)
    ...
    def from_3_points(p_neg_x_axis, origin, p_xy_plane, eps=1e-8):
        ...
        return Frame(rots, torch.stack(origin, dim=-1))
    ...
    @staticmethod
    def stack(ts, dim):
        rots = torch.stack([t.rots for t in ts], dim=dim if dim >= 0 else dim - 2)
        trans = torch.stack([t.trans for t in ts], dim=dim if dim >= 0 else dim - 1)
    ...
    def stop_rot_gradient(self):
        return Frame(self.rots.detach(), self.trans)

    def scale_translation(self, factor):
        return Frame(self.rots, self.trans * factor)
    ...
    def make_transform_from_reference(n_xyz, ca_xyz, c_xyz, eps=1e-20):
        ...
        return Frame(rots, translation)

    def cuda(self):
        return Frame(self.rots.cuda(), self.trans.cuda())
